...
dataFrame.imputed<-function(subData.impute,missing_cells){
  subData.impute<-subData.impute[1,]
  subData.imp<-subData.impute[[1]]
  for(i in 2:length(subData.impute)){
    subData.imp<-rbind(subData.imp,subData.impute[[i]])
  }

  imputed<-c()
  for(i in 1:nrow(missing_cells)){
    imputed<-c(imputed,subData.imp[missing_cells[i,]$rownames,missing_cells[i,]$colnames])
  }
